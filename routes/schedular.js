...
let translatorRouter  = require('../src/app/controllers/TranslatorController');
...
function getDataTrasactionMode() {
    ...
    return db.task('getApiSettingsInformation', t => {
        return t.oneOrNone('SELECT mode_type FROM exchange_mode')
            .then(info => {
            	return info;                
            }).catch(error=> {
        		console.log("Information not found");
							...
							// Running the cron job in every five minutes 
								...
								// Only Source is DHIS
								  ...
								  if(data.instance_type=='dhis2'){
								  	...
								  	for (let i = 0; i < schedularTask.length; i++) {
								  	    //console.log(i);	  	
								  		translatorRouter.processMessages(schedularTask[i], modeType,exportLimit,exportFromDays);
								  	}
